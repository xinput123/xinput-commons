...
import com.github.xinput.commons.consts.RedisConsts;
  ...
  public static JedisPoolConfig getPoolConfig() {
    ...
    for (int i = 0; i < methods.length; i++) {
      ...
      if (methodName.startsWith("set")) {
        String prop = methodName.replaceFirst("set", StringHelper.EMPTY);
        ...
        if (value != null) {
          ...
          ReflectHelper.on(jedisCfg)
              .call(methodName, TypeHelper.cast(value, method.getParameterTypes()[0]));
        }
      }
    }
    return jedisCfg;
